...
namespace QuanLiCH
{
    public partial class fTableManager : Form
    {
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            int idBill = BillDAO.Instance.GetUncheckBillIDByTableID(table.ID);
            double discountPercentage = Convert.ToDouble(nmDisCount.Value);

            double totalPrice = Convert.ToDouble(txbTotalPrice.Text.Split(',')[0]);
            double discountAmount = (totalPrice / 100) * discountPercentage;
            double finalTotalPrice = totalPrice - discountAmount;

            if (idBill != -1)
            {
                if (MessageBox.Show(string.Format("Bạn có chắc thanh toán hóa đơn cho bàn {0}\nTổng tiền - (Tổng tiền / 100) x Giảm giá\n=> {1} - ({1} / 100) x {2}% = {3}", table.Name, totalPrice, discountPercentage, finalTotalPrice), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                {
                    BillDAO.Instance.CheckOut(idBill, (int)discountAmount, (float)finalTotalPrice);
                    ShowBill(table.ID);
